...
def clicked():
    try:
        ...
        entry.delete(0, 'end')
        data={"APPID":'05cfc6b58e66dc1c215c660ad12c19fa',"q":user_entry,"units":"imperial"}
        
        # Get information From OpenWeather API Website 
...
def refine(day_info,dates):
    ...
    for x in day_info[:len(day_info)-16]:
        var = (x[0] -32)*5/9
        res = "{:.2f}".format(var)
        today+=x[2]+" - "+res+" "+degree+"C"+" - "+x[1]+"\n\n"
    for x in day_info[len(day_info)-16:len(day_info)-8:]:
        var1 = (x[0] -32)*5/9
        res1 = "{:.2f}".format(var1)
        tomorrow+=x[2]+" - "+res1+" "+degree+"C"+" - "+x[1]+"\n\n"
    for x in day_info[len(day_info)-8:]:
        var2 = (x[0] -32)*5/9
        res2 = "{:.2f}".format(var2)
        day_after+=x[2]+" - "+res2+" "+degree+"C"+" - "+x[1]+"\n\n"
